...
public class OpportunitySpecification {
	public static Specification<Opportunity> getOpportunitySpecification(SearchRequestVO searchRequest) {
		return new Specification<Opportunity>() {
			...
			public Predicate toPredicate(Root<Opportunity> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				try {
					final Predicate deletedPredicate = cb.equal(root.get(Opportunity_.deleted), false);
					...
					if (!filters.isEmpty()) {
						filters.forEach(item -> {
							...
							if (!StringUtils.isEmpty(searchText) && colName.equals("name")) {
								final Predicate localpredicate = cb.like(cb.lower(root.get(Opportunity_.name)),
										"%" + searchText + "%");
							...
							if (!StringUtils.isEmpty(searchText) && colName.equals("startDate")) {	
								try {
									...
									final Predicate localpredicate = cb.equal(root.get(Opportunity_.startDate), date);
									predicates.add(localpredicate);
								} catch (Exception e) {
							...
							if (!StringUtils.isEmpty(searchText) && colName.equals("endDate")) {
								try {
									Date date= formatter.parse(searchText);
									final Predicate localpredicate = cb.equal(root.get(Opportunity_.endDate), date);
									predicates.add(localpredicate);
								} catch (Exception e) {
									throw new OMSSystemException(Status.FAILED.name(), HttpStatus.CONFLICT,
											"Unable to format date ", e);
							...
							if (!StringUtils.isEmpty(searchText) && colName.equals("dueDate")) {
								try {
									Date date= formatter.parse(searchText);
									final Predicate localpredicate = cb.equal(root.get(Opportunity_.dueDate), date);
									predicates.add(localpredicate);
								} catch (Exception e) {
									throw new OMSSystemException(Status.FAILED.name(), HttpStatus.CONFLICT,
											"Unable to format date ", e);
							...
							if (!StringUtils.isEmpty(searchText) && colName.equals("salesCategory")) {
								final Predicate localpredicate = cb.equal(
										root.join(Opportunity_.salesCategory).get(SalesCategory_.salesCatagoryId),searchText);
								predicates.add(localpredicate);
							}
							if (!StringUtils.isEmpty(searchText) && colName.equals("advertiser")) {
								final Predicate localpredicate = cb.equal(
										root.join(Opportunity_.advertiser).get(Company_.companyId), searchText);
								predicates.add(localpredicate);
							}
							if (!StringUtils.isEmpty(searchText) && colName.equals("agency")) {
								final Predicate localpredicate = cb.equal(
										root.join(Opportunity_.agency).get(Company_.name), searchText );
								predicates.add(localpredicate);
							}
						...
						columns.forEach(item -> {
							...
							if (!StringUtils.isEmpty(searchText) && columnName.equals("budget")) {
								final Predicate localpredicate = cb.equal(root.get(Opportunity_.budget), searchText);
								predicates.add(localpredicate);
							}
							if (!StringUtils.isEmpty(searchText) && columnName.equals("percentageOfClose")) {
								final Predicate localpredicate = cb.equal(root.get(Opportunity_.percentageOfClose),
										searchText);
								predicates.add(localpredicate);
							}
							if (!StringUtils.isEmpty(searchText) && columnName.equals("submitted")) {
								boolean enabled = searchText.equals("true");
								final Predicate localpredicate = cb.equal(root.get(Opportunity_.submitted), enabled);
								predicates.add(localpredicate);
							}
						});
					}
