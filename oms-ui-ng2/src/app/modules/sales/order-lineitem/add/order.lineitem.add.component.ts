...
import { OrderLineItem, LineItemOrder } from '../../../../common/models/order.lineitem';
...
@Component({
  templateUrl: './order.lineitem.add.component.html',
  styleUrls: ['./../order.lineitem.component.scss'],
  providers: [SalesService],
})
export class OrderLineitemAddComponent implements OnInit {
  ...
  currentLineItem: OrderLineItem = new OrderLineItem();
  ...
  cretiveOptions: IMultiSelectOption[];
  cretiveOptionsModel = [];
  ...
  loadProposalData() {
    ...
    if (lienItemId !== undefined) {
      ...
      let _selectedLineitem = this.newOrder.lineItems.filter(function (value) {
        if (value['orderLineItemId'] == lienItemId) {
          return value;
        }
      })[0];
  ...
  backToTable() {
    ...
    let param = this.activatedRouter.snapshot.queryParams["callback"];
    if (param === 'addProposal') {
      this.router.navigate(['dashboard/sales/order/view'], { queryParams: { callback: 'addProposal', proposalItem: JSON.stringify(this.newOrder) } });
    } else {
      this.router.navigate(['dashboard/sales/order/view'], { queryParams: { proposalItem: JSON.stringify(this.newOrder) } });
    }
  }
  ...
  saveForm(formData: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (this.getSetService.obj != undefined) {
        this.newOrder.action = this.getSetService.obj;
  ...
  saveProposalData(formData: any) {
    ...
    //   }
    if (this.cretiveOptionsModel.length > 0) {
      var _selectedCretiveId = this.cretiveOptionsModel[0];
      let _selectedCretive = this.cretiveList.filter(function (value) {
        if (value['id'] == _selectedCretiveId) {
          return value;
        }
      })[0];
      this.currentLineItem.creativeAsset = _selectedCretive;
      this.currentLineItem.cretive = _selectedCretive['name'];
    }
    ...
    //   }
    // }
    this.newOrder.lineItems = [];
    this.newOrder.lineItems.push(this.currentLineItem);
  ...
  updateLineitem() {
    ...
    let lineitemOrder = new LineItemOrder();
    lineitemOrder.name = this.newOrder.name;
    lineitemOrder.orderId = this.newOrder.orderId;
  ...
  resetForm() {
    if (this.editingLineItemId) {
      ...
      this.currentLineItem = new OrderLineItem();
  ...
  loadAllDeliveryInfo() {
    ...
    let cretiveUrl = PathConfig.BASE_URL_API + PathConfig.CRETIVEASSET_LIST;
    ...
    var cretiveData = this.httpService.get(cretiveUrl, auth_token);
              ...
              rotateCreativeData => {
                ...
                priorityData.subscribe(
                  priorityData => {
                    ...
                    cretiveData.subscribe(
                      cretiveData => {
                        if (cretiveData) {
                          this.cretiveList = cretiveData;
                          this.cretiveOptions = DropDownDataFilter.convertDataIntoDropdown(cretiveData, 'id', 'name');
                        }
  ...
  populateLineitemData() {
    ...
    if (this.currentLineItem.creativeAsset != null) {
      this.cretiveOptionsModel[0] = this.currentLineItem.creativeAsset.id;
    }
    //  this.productOptionsModel[0] = this.currentLineItem.product.productId;
    //this.specTypeOptionsModel[0] = this.currentLineItem.specType.spec_Id;
