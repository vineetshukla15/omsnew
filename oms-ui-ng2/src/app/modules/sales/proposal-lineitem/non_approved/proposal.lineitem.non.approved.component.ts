...
@Component({
  templateUrl: './proposal.lineitem.non.approved.component.html',
  styleUrls: ['./proposal.lineitem.non.approved.component.scss'],
})
export class ProposalLineitemNonApprovedComponent implements OnInit {
  ...
  backToTable() {
    ...
    if (this.newProposalLineItem.lineItemId === undefined)
      ...
      if (param === 'addProposal') {
        this.router.navigate(['dashboard/sales/non-approved-proposal/edit'], { queryParams: { callback: 'addProposal', proposalItem: JSON.stringify(this.newProposal) } });
      } else {
        this.router.navigate(['dashboard/sales/non-approved-proposal/edit'], { queryParams: { proposalItem: JSON.stringify(this.newProposal) } });
      }
    }
  }
