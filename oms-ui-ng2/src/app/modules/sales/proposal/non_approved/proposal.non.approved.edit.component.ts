...
@Component({
  templateUrl: './proposal.non.approved.edit.component.html',
  styleUrls: ['./../proposal.component.scss'],
})
export class ProposalNonApprovedEditComponent implements OnInit {
  ...
  assignTask() {
    let findTaskUrl = PathConfig.BASE_URL_API + PathConfig.TASK_GET_ID + this.newProposal.proposalId + "/" + this.newProposal.assignTo.id;
    let auth_token = this.localStorage.get(ConstantConfig.AUTH_TOKEN);
    ...
    data.subscribe(
      data => {
        if (data) {
          ...
          let assignTaskUrl = PathConfig.BASE_URL_API + PathConfig.ASSIGN_TASK + processId + "/assignTask/" + taskId + "/" + this.newProposal.assignTo.id;
          ...
          AssignedTaskData.subscribe(
            AssignedTaskData => {
              if (AssignedTaskData) {
                ...
                this.router.navigate(['dashboard/sales/proposal/edit'], { queryParams: { callback: 'editProposal', proposalId: this.newProposal.proposalId } });
              }
            }, error => {
              if (error) {
  ...
  updateProposal() {
    ...
    let auth_token = this.localStorage.get(ConstantConfig.AUTH_TOKEN);
    this.proposalId = this.newProposal.proposalId;
  ...
  addNewLineitem() {
    let param = this.activatedRouter.snapshot.queryParams["callback"];
    if (param === 'addProposal') {
      this.router.navigate(['dashboard/sales/proposal-lineitem/add'], { queryParams: { callback: 'addProposal', proposalItem: JSON.stringify(this.newProposal) } });
    } else {
      this.router.navigate(['dashboard/sales/proposal-lineitem/add'], { queryParams: { proposalItem: JSON.stringify(this.newProposal) } });
    }
  }
  ...
  editLineitem(lineitem) {
    ...
    let param = this.activatedRouter.snapshot.queryParams["callback"];
    if (param === 'addProposal') {
      this.router.navigate(['dashboard/sales/non-approved-proposal-lineitem/edit'], { queryParams: { callback: 'addProposal', proposalItem: JSON.stringify(this.newProposal), 'lineitemId': id } });
    } else {
      this.router.navigate(['dashboard/sales/non-approved-proposal-lineitem/edit'], { queryParams: { proposalItem: JSON.stringify(this.newProposal), 'lineitemId': id } });
    }
  }
